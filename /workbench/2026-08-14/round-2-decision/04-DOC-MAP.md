...
| Definition＋Call→Task | 核心公式 | Call identity/resolver、Task acceptance | CAS/ID options |
...
- [`AOS-ARCHITECTURE.md`](../../../AOS-ARCHITECTURE.md)：移除 `Function = process call`、`Task = ordered Steps`、`Step = durable process Call`；改用 Call identity、Task execution instance、Return/Receipt、root registry 與 relation authority。
- [`AOS-SCHEDULING.md`](../../../AOS-SCHEDULING.md)：以 accepted root/child Tasks、eligible boundary 與 serialization key 描述；central Function registry 不進 core。
