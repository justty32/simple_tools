...
**使用者已定：**Step 與 Round 都是 Function；Tool 是 Function 在 Agent tool 語境中的角色。Tool Binding 把模型 Tool Call 轉成純 AOS Function Call，AOS 接受後才建立 Tool Task。
...
- binding 產生的 Call 只含已解析 Definition、argv、stdin bytes、cwd、environment policy等；不能放 model call ID、Task ID、parent、slot 或 state。模型配對 metadata 留 Agent 層。
...
- Task Receipt 必須綁 `task_id + call_hash + basis`。leaf basis 指向 attempt；composite basis 是 ordered child Receipts。
...
- **P1a-2 待驗：**accept/planned前把相對 Definition依 caller cwd解成 absolute executable path，以入口檔 content SHA-256作 generation並存入 Call。
...
同一 Definition 可有 human-friendly CLI 與穩定 machine mode。Machine primitive 始終使用明確 path；`aos exec ./ask`、`aos exec .`、PATH sugar 的 UX 留 P1c，sugar 必須先解析再 accept。
