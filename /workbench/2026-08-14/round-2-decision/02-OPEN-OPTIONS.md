...
**P1 驗證證據：**Call不含 relation、每 Task自存 Call、parent ordered events是 child relation authority、unknown無 Receipt。**內部+Opus 暫選、待 P1a-2 驗證：**`call_ref{hash,size}`作長期 identity介面、root registry作 root authority、B1 planned才 freeze與 B4 GC規則。P1a-2不另選形狀，但必須先實作 B1/B2。
...
兩案都必須維持 `call_ref{hash,size}`，且相同 Call 可產生不同 Tasks。P1a-2 不做 CAS。
...
ID 必須獨立於 Call identity，否則同一 Call 無法形成不同 Tasks。P1 也不承諾 public `accept` 的 idempotency／request ID；若產品需要重送安全，須另定 client request key、期限與 collision policy。
...
Machine 接受明確 path 已定；surface 仍待 P1c：
...
PATH sugar 必須先解析出明確 path，再呼叫同一 primitive。directory module 的固定 `./call`、內部 manifest 或 façade＋manifest 仍待比較；不得猜 `main.py`。
