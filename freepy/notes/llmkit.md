# llmkit 的實測筆記（2026-08-05 ~ 08-08）

從 [NOTES.md](README.md) 拆出來的。這些是 `llmkit/` 定型**之前**踩出來的東西 ——
為什麼 `Reply` 長這樣、串流有哪兩個坑、`ask()` 修過什麼、模型清單和 caps 怎麼問出來的。

都已經結案，不會再改，但改 `llms/` 之前值得回頭看一眼，免得把修好的洞再挖一次。
還在動的東西留在 NOTES.md。
