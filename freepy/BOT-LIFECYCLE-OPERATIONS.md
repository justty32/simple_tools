...
目前 repo 已有可用的觀測地基：`llms` 把 provider usage 正規化成 `usage["cached"]`；DeepSeek 卡實測可觀察 cache hit，Ollama／LM Studio 經 proxy 的卡則是 false／看不到指標。這只能表示「呼叫端能否觀察」，不能反證 runner 內部完全沒有 KV reuse；modelcards 尚在審核，scheduler 要接受 `true | false | unknown`。
...
- endpoint error：按 endpoint policy backoff；Step attempt／token cost留紀錄，只提交一個 response。
