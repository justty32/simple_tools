...
「這個參數該怎麼用」。多打幾行字換到的是模型真的看得懂的描述。

`params` 直接就是 JSON Schema 的 properties，不發明第二套 DSL：這樣要加 `enum`、
`minimum`、`items` 都是照 JSON Schema 寫，不用等這個 lib 支援。
...
def _module_name(cls):
    """class 自己說的模組名。`"__main__"` 要從檔名反推，那個名字別人 import 不到。
    ...
    **不要猜一個** —— 猜錯的話 .json 現在存得出來，等別的行程去讀才炸。
    ...
    if name != "__main__":
        return name
    stem = os.path.splitext(os.path.basename(_source_file(cls)))[0]
    need(stem and stem != "__main__",
