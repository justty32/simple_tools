...
```bash
PYTHONPATH=.:llmkit uv run python agentloop/repl/scripts/check_pi_bridge.py
```

這會用 `pi_minimal_factory.py` 驗證：
...
```bash
AGENTLOOP_PI_FACTORY=agentloop.repl.examples.pi_minimal_factory:create \
pi -e freepy/agentloop/repl/scripts/pi-agentloop.ts
```
