...
```text
Turn      一次完整 run()
Round     一次 ask()，到完整收到 message 或 endpoint error
tool call 發生在 Round 之間，不另算 Round
```

request 送出即計一個 Round；timeout/error 也算，transport 內部 retry 是另一個 attempt
維度，不另算 Round。`Handle.round` 與 `Limits.rounds` 維持目前語意；歷史命名留下的工具
紀錄 `Handle.steps` 應改成 `tool_log`。完整定義以 [TURNS.md](TURNS.md) 為準。
...
- tool input、`turn_id`、event trace、跨 process control plane。
...
Round，空才 `done`。提交後追加明確回 `turn already completed`。

### P0：tool 中的 pause／stop 會多放行一個 Round
...
必要例外：tool batch 中 stop 時，結果要先用一個 settlement Round 餵回 bot，否則續跑會重做 history 中仍 pending 的副作用：

```text
stop during model → 收完本 Round；不執行新 tool calls；stopped
stop during tools → 跑完整批 → 送回全部 results 一個 Round → 不執行新 calls；stopped
...
文件允許其他 thread 控制，但 phase、flags、`Handle.steps`、`_say` 都是裸 field/list；GIL 不等於完成/enqueue 原子性。
...
現有 `Limits(rounds=None)` 直到 `range(None)` 才炸；負數、錯型別也延後，`quiet=0` 還被無聲改成 1。

要求：建構時驗證。`rounds` 是正整數；`calls`、`seconds`、`tokens` 是 `None` 或非負值；per-tool cap 非負；`quiet >= 1`。錯誤直接 `ValueError`。
...
- 一次 `run()` 是一個 Turn；一次 endpoint request/response 是一個 Round。
- tools 在兩個 Round 之間執行，不增加 round。
- 一個 Handle 只屬於一個 Turn；同一 bot 可在下一 Turn 用新 Handle 延續 history。
- 同一 bot 同時跑兩個 Turn 不支援；budget/stopped 後再次 `run()` 是新 Turn。
...
2. **Handle + 命名**：round/tool_log、lock、instruction FIFO、完成提交、one-shot guard。
...
6. **整理文件**：README 與規劃統一採用 `TURNS.md` 的 Turn／Round；OS sandbox 不實作。
...
- final-response 與 instruction 同時抵達：instruction 被下一 Round 看見，或在 completion 提交後明確拒絕。
...
- Handle 重用、非法 Limits 立即失敗，沒有半啟動 Turn。
...
2. **stop during tools 要有 settlement Round**：為了不重跑副作用，允許多一次模型 request，但不再執行它新要求的工具。
