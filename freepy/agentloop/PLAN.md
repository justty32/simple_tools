...
本次**不動根基**：llms、proxy、tooljson 介面 freeze；modelcards 等審核。Plan 9、多 agent、memory、runtime、固化等都不是 v1 前置條件。
...
```text
Round     一次完整 run()
Step      向思考引擎 endpoint 發出 request，到完整收到 response 或 endpoint error
tool call 發生在 Step 之間，不另算 Step
```

request 送出即計一個 Step；timeout/error 也算，transport 內部 retry 不另算。實作時 `Handle.round → step`、`Limits.rounds → steps`；工具紀錄 `Handle.steps → tool_log`。`round` 改指整次 `run()`，不留舊欄位 alias。
...
要求：enqueue 與 completion commit 共用 lock。無工具時先原子檢查 queue；有指令就開下一 Step，空才 `done`。提交後追加明確回 `round already completed`。
...
要求：建構時驗證。新 `steps` 是正整數；`calls`、`seconds`、`tokens` 是 `None` 或非負值；per-tool cap 非負；`quiet >= 1`。錯誤直接 `ValueError`。
...
- 一次 `run()` 是一個 Round；一次 endpoint request/response attempt 是一個 Step。
...
2. **Handle + 命名**：step/steps/tool_log、lock、instruction FIFO、完成提交、one-shot guard。
...
6. **整理文件**：README 吸收 Round/Step；`TURNS.md` 術語失效，future 內容另歸檔或刪除；OS sandbox 不實作。

不新增第三方依賴，不改 llms/tooljson/modelcards，不趁機抽 framework。
