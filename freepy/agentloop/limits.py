...
那幾種：跑幾輪、叫幾次工具、哪些工具、花多少 token、跑多久。
...
兩種擋法，分得很清楚：
    ...
    停整個 agent      預算真的沒了（輪數、時間、token、總呼叫數）
...
class Limits:
    ...
    def __init__(self, rounds=12, calls=None, per_tool=None, tools=None,
                 engines=None, seconds=None, tokens=None, quiet=1):
        #: 模型最多講幾輪。這是唯一有預設值的 —— 沒有它，壞掉的模型會永遠跑下去
        self.rounds = rounds
        #: 工具總共最多叫幾次
        self.calls = calls
        ...
        #: 最多花掉多少 token，模型每輪自己回報的加總
        self.tokens = tokens
        #: 連續幾輪不叫工具就當它講完了。預設 1 = 一不叫工具就收工。
        #: 調大就會推它一把再給幾次機會 —— 小模型很常直接講一段話而不動手
        self.quiet = max(1, quiet)

    def exhausted(self, h):
        """預算沒了嗎？回停的原因，還能跑就 `None`。每輪開頭問一次。"""
    ...
    def engine_ok(self, bot):
        ...
        每輪開頭都問一次，不是只問開頭那一次 —— 引擎是 `bot.engine.model` 一個
