...
「一萬個 bot，不等於一萬個常駐 agent process」這個判斷成立。bot 的 system prompt、tool JSON 多半是不可變資料，history 是持久化、持續追加的資料；只有 Round 活躍時，才需要取出上下文並送 Step 給 thinking endpoint。冷 bot 可以只是一筆磁碟上的 record。
...
一個 Step 結束後更新 history 與 Round record：若有工具，送 tool queue；工具結果完整落盤後，Round 才重新成為 runnable。若是 final response／error／budget，進 terminal state。這恰好沿用目前的 Round／Step／tool batch 語意，而不是另造一套 loop。
...
要特別防「thundering herd」：endpoint 恢復時不能讓數千 Round 同時 retry。rate budget 與 jitter 應屬 endpoint scheduler，不散落在每個 bot 裡。
...
這個規模假設會影響未來資料模型，**不必打斷當前 agentloop 收尾**。目前只要讓其語意不封死未來：Round 與 Step 已分開；Handle 控制有安全邊界；bot/dispatch 是介面。`round_id`、durable queue、worker lease、HTTP、FUSE/9P 都等 agentloop 審完再設計。

最合理的下一個 scale prototype，是用 fake endpoint 跑「一萬個 dormant bot records、幾百個 queued Rounds、固定 K 個 active Steps」，量測 scheduler memory、fairness、crash recovery；不是開一萬個真 CLI，也不是先做漂亮 dashboard。
