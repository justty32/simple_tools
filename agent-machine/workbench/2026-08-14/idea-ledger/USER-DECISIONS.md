...
- 同一 agent root 正忙碌時，普通新呼叫預設直接拒絕並清楚說明「正在忙」；被拒絕的呼叫不會自動建立 Task。
- 使用者可以用明確操作，把新的 prompt 追加到目前 Task；這不同於普通新呼叫。
- 使用者也可以用另一個明確操作建立新的排隊 Task，但必須同時註明：目前 Task 到了什麼狀況，排隊 Task 才可開始。
- 條件式排程必須考慮同一 agent root 內多個 Task 並行會弄亂資料夾；不能只看前一個 Task 的狀態就直接啟動。
- 若同一 root 同時有兩個 Task 在執行，資料夾內應能看見目前 active Tasks；這份資訊由 AOS 管理，讓在該地盤工作的 agent 知情。
