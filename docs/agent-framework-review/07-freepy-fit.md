...
| Instructor schema/re-ask | llm adapter、task report、tooljson | 每次 re-ask 算新 Round |
...
## Turn、Round、step、attempt

現有 [Turn/Round 定義](../../freepy/agentloop/TURNS.md) 比外部框架更清楚，應保持：

- 使用者啟動到 agent 主動停止是一個 Turn；
- 每次真正 provider ask→message 是一個 Round；
- Instructor validation re-ask 是新 Round；
- Swarm loop 的 history message counter 不是 Round；
- LangGraph superstep、PocketFlow node、CrewAI task 都是 workflow step，可能含零到多個 Round；
- retry attempt 是另一維度，不因重試便偷改 Turn/task identity；
- tool 內部 human input 仍是 pending tool event，不增加 Round。

每個 event 同時可帶 `turn_id, round_no, workflow_run_id, step_no, task_id, tool_call_id, attempt_no`，不用硬把它們壓成一個數字。

## agentfs 應投影什麼

在既有 [agentfs 規劃](../../freepy/agentfs/PLAN.md) 上增加 read-only views：
...
4. 每個 Round 保存實際 context manifest；
5. memory ref 仍做 ACL 檢查，知道 ref/path 不等於獲准讀取。

checkpoint 只保存 workflow cursor/state ref，不複製整份模型 history。source trace、memory object、Round manifest 與 checkpoint 可以互相連結，但生命週期各自獨立。

## Runtime 與安全

七個框架的 schema、guardrail、router、checkpoint 都是應用層技術，不能限制 process 真正的檔案/網路/CPU 權限。[agent runtime 規劃](../../freepy/agent_runtime/PLAN.md) 的 supervisor、effective policy、rootless container/cgroup 與無 runtime socket 原則仍需保留。
