...
> agent 是在私有 namespace 裡求值的 process；agentfs 是它看得見的世界；memory 是可掛載的持久結構；context 是每一輪按需編譯出的 working set。
...
context     = 本 Round 的 manifest + working set
Round       = ask(...) -> model message
Turn        = agent 啟動後到主動停止的一段活動
...
- 認知層保留不可變 trace，把舊段落轉成可尋址記憶，為每個 Round 編譯有限 context。
...
6. **context compact 是重新編譯 view。** 原始 user／assistant／tool trace 不被刪改；下一輪只帶 inline blocks、link cards 與明確載入的 refs。
...
4. [Round、Turn 與 tick](04-time-agency.md)
5. [檔案協定、能力與隔離](05-protocol-security.md)
6. [分期路線與驗收](06-roadmap.md)

瀏覽器入口是 [index.html](index.html)。較貼近目前程式碼的實作規劃仍在 `freepy/agentfs/`、`freepy/memory_tools/` 與 `freepy/agentloop/TURNS.md`。
