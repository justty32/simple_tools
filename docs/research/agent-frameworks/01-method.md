...
- `docs/freepy/future/agent-runtime/PLAN.md`：spawn、policy derivation、instance lifecycle、sandbox。
- `docs/freepy/future/team-tools/PLAN.md`：agent path、task、grant、resource ledger。
- `docs/freepy/future/memory-tools/`：object/ref、organized context、Step manifest。
- `docs/freepy/future/agentfs/PLAN.md`：只讀 synthetic filesystem projection。

後四項均是延後候選規格，不是目前 package 或排程。未實作的 `PLAN.md` 不會被誤寫成現成功能；
本報告只判斷哪些外部做法能縮短正確實作路徑。
