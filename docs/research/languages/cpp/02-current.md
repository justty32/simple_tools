...
| `prototypes/exec_tools/discover` | env path scan、spec lookup、missing/errors 的草稿 | A/B | 規則先凍結，再評估 C++ |
| `shells` | chdir、PYTHONPATH、`execvpe` launcher | D | 依部署重做，不逐行移植 |
| prototypes／`examples/llm_tool_roundtrip.py` | 互動探索與 demo | D | 繼續 Python |
