...
邊界見 [Agent Machine](../agent-machine/README.md) 與
...
不用直接做 `/root/build/env/...`，因為 agent child 也可能叫 `env`、`status` 或 `tasks`；單一保留的 `.agent` 可以永久隔開「組織子樹」與「agent 自己的檔案介面」。規劃中、尚未建立的 `agent_identity.py` 必須禁止 child segment 恰好為 `.agent`。
...
1. 新增 `agent_identity.py`，並保留 `.agent` segment。
