...
> **一半已經落地到 [`../llmkit/tooljson/`](../../../../freepy/llmkit/tooljson/README.md)。**
...
最初的攤平 JSON、argv mapping 與 source fingerprint 設計已移到
[`SPEC-DRAFT.md`](../../../archive/freepy-exec-tools-spec-draft.md) 保存。它已被 `llmkit/tooljson` 的正式格式取代，不能當成
目前 API；保留只是為了解釋早期設計如何演進。
