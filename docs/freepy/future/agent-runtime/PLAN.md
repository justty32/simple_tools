...
path 是人與模型看到的邏輯組織身分，不是宿主 filesystem path；若日後實作，三層應共用一個尚未建立的 `agent_identity.py` 解析它。runtime 另配不可重用的 `instance_id`。同一路徑的 agent 結束後即使允許重建，也不能讓舊 event、reservation 或 mailbox ack 誤套到新 instance。v1 直接禁止 active path 重複與 reparent；移動子樹等於改身分，留到有 migration protocol 再做。
...
3. 新增共用 `agent_identity.py`：canonical path、parent/ancestor、segment-safe subtree 判斷。
