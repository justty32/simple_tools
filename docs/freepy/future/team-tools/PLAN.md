...
這個設計借 Unix namespace 的好處：身分、隸屬與 scope 用同一個可讀地址表達。但它是邏輯 path，不是宿主路徑；若日後實作，三層應共用一個尚未建立的 `agent_identity.py` 解析。**path 只表示位置，不自動授權**；一個叫 `/root/admin` 的 agent 若沒有 grant，仍沒有管理能力。
...
1. 新增規劃中的共用 `agent_identity.py` canonical path parser。
