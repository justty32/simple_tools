...
**狀態：延後可行性筆記。** 這份保留 scale 與介面推導；執行模型已整理進
[Agent Machine 候選規格](../agent-machine/README.md)。實際優先序只以
[ROADMAP.md](../../../../freepy/ROADMAP.md) 為準，目前未啟動 durable control plane。

## 判斷

bot 從磁碟喚醒、待命、沉睡，以及 leader 的控制流程見 [lifecycle](lifecycle.md)。
...
這不是現在的下一步。若 ROADMAP 所列的 durable 需求真的出現，可先用 Agent Machine M0 + M1
做一個 scale prototype：以 fake endpoint 跑「一萬個 dormant bot records、
