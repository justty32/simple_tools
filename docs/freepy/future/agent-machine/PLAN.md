...
**狀態：延後候選計畫。** 這裡保存「若日後出現跨重啟、跨 process 與可靠排程需求，可能怎麼做」；
它不是目前 sprint，也不能覆蓋 [`freepy/ROADMAP.md`](../../../../freepy/ROADMAP.md) 的順序。
...
## M2：接上 agentloop Round adapter

`agentloop` 的 lock、safe boundary 與 settlement 修正已完成；現行語意見
[`agentloop/ROUNDS.md`](../../../../freepy/agentloop/ROUNDS.md)。本地 `advance()` 已能執行一個 Step
或一整批 tools 並抵達下一個 safe boundary；`run()` 是反覆驅動同一 runner 的便利 API。

若本計畫啟動，應新增外圍 Agent Machine adapter，把本地邊界與結果映射成 `AskStep`、
`RunToolBatch`、`Candidate` 等 durable work records。這不是 `advance()` 的回傳型別，也不把
durability 塞回 Controller；scheduler 不得直接修改 `LLM.history`。
...
- 本文件只保存 Agent Machine 內部的候選順序；[`ROADMAP.md`](../../../../freepy/ROADMAP.md) 負責
  全 repo 的實際優先序，且目前明確延後本計畫。
- Round／Step 術語仍以 [`../agentloop/ROUNDS.md`](../../../../freepy/agentloop/ROUNDS.md) 為準。
- agentfs、memory、runtime、team 的局部候選不變量仍由各自 `PLAN.md` 描述；這些文件彼此衝突時
  必須在真正開工前重新決策，不能只靠檔案新舊決定權威。
